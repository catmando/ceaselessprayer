...
#{"\n**Bookmark us for quick loading, and automatic updates.**" if PWA.ready_to_install?}
MARKDOWN
  end
  ...
  styles do
    if WindowDims.height > 750 && WindowDims.width > 400
      ...
      { container: style.merge(opacity: 0.8, fontSize: 12), paper: { padding: 5, marginTop: 5 } }
    end
  end

  render do
    DIV(styles(:container), class: 'row content') do
      papers
    end
  end
